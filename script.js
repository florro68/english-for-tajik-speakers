...
function chooseVoice(){
  ...
  return v.find(x=>x.lang.startsWith("en")&&/Google|Microsoft|Samantha|Daniel|Zira|David/i.test(x.name))
      ||v.find(x=>x.lang.startsWith("en"))||v[0];
}
function speakText(text,rate=.86){
  speechSynthesis.cancel();
  currentText=text;currentRate=rate;
...
function speakWithPauses(items,pauseMs=3000,rate=.78){
  speechSynthesis.cancel();let i=0;
  currentText=items.join(". ");currentRate=rate;
  function next(){
